...
export default class Controller {
  ...
  findById() {
    ...
    function find(model) {
      return (ctx, next)=> {
        const query = _buildQuery_(model.findOne(), ctx.request.query, ctx.params);
  ...
  removeById() {
    ...
    function remove(model) {
      return (ctx, next)=> {
        const query = _buildQuery_(model.remove(), ctx.request.query, ctx.params);
...
function _buildQuery_(fn, query = {}, params = {}) {
  const id = params.id;
